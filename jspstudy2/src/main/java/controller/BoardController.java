...
public class BoardController extends MskimRequestMapping {
	...
	public String write(HttpServletRequest request, HttpServletResponse response) {
		...
		if (dao.insert(board)) {
			return "redirect:list?boardid=" + boardid; // 등록되면 list로 전달
		}
		// 게시물 등록 실패시 실행되는 부분
	...
	/*
	 * 1. num 파라미터 저장. session에서 boardid 조회하기. 2. num값의 게시물을 db에서 조회. Board b =
	 * BoardDao.selectOne(num) 3. num값의 게시물의 조회수 증가시키기 void BoardDao.readcntAdd(num)
	 * 4. 조회된 게시물 화면에 출력.
	 */
	@RequestMapping("info")
	...
	/*
	 * 1.원글의 num을 파라미터 저장 : num 원글의 게시물번호 2.db에서 num의 게시물 조회하여 원글의
	 * num,grp,grplevel,grpstep ,boradid 정보를 저장 3.입력 화면 표시
	 */
	...
	/*
	 * 1.파라미터 값을 Board 객체에 저장하기 원글정보 : num, grp, grplevel, grpstep, boardid 답글정보 :
	 * writer, pass, title, content => 등록정보 2. 같은 grp 값을 사용하는 게시물들의 grpstep 값을 1 증가
	 * 하기. void BoardDao.grpStepAdd(grp,grpstep) 3. Board 객체를 db에 insert 하기. num :
	 * maxnum +1 grp : 원글과 동일. grplevel : 원글 + 1 grpstep : 원글 + 1 boardid : 원글과 동일
	 * 4. 등록 성공시 : list.jsp로 페이지 이동 등록 실패시: "답변등록시 오류발생" 메세지 출력 후, replyForm.jsp로
	 * 페이지 이동
	 */
	...
	public String reply(HttpServletRequest request, HttpServletResponse response) {
		...
		b.setNum(++num); // 답글정보
		b.setGrplevel(grplevel + 1); // grplevel : 원글의grplevel// grplevel+1=>답글데이터
		b.setGrpstep(grpstep + 1); // 원글 grpstep + 1 => 답글데이터
		if (dao.insert(b)) {
	...
	public String deleteForm(HttpServletRequest request, HttpServletResponse response) {
		...
		if (boardid.equals("1")) {
			if (login == null || !login.equals("admin")) {
				...
				request.setAttribute("msg", "list?boardid=1");
				return "alert";
			}
		}
	...
	/*
	 * 1. num, pass 파라미터를 변수에 저장. 2. 비밀번호 검증 틀린경우 : 비밀번호 오류 메시지 출력, deleteForm.jsp
	 * 페이지 이동 3. 해당게시물이 공지사항 게시물인 경우 관리자만 삭제 가능 4. 게시물삭제 boolean
	 * BoardDao.delete(num) 삭제 성공 : list.jsp 페이지 이동 삭제 실패 : 삭제 실패 메세지 출력, info.jsp
	 * 페이지 이동
	 */
	...
	public String imgupload(HttpServletRequest request, HttpServletResponse response) {
		...
		String fileName = multi.getFilesystemName("upload"); // ckeditor에서 upload로 고정돼있음 이미지속성에서 파일선택 이름이 업로드임
	...
	public String comment(HttpServletRequest request, HttpServletResponse response) {
		...
		comm.setSeq(++seq);// 나의 번호는 최대값 보다 하나 커야함
